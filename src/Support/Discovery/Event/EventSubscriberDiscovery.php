...
use Cline\Discovery\Attribute\Event\AsEventSubscriber;
use ReflectionClass;
 ...
 */
final class EventSubscriberDiscovery
{
    /**
    ...
    private static function discoverViaComposerClassmap(): array
    {
        ...
        foreach ($classmap as $class => $file) {
            ...
            if (!str_contains($path, '/Subscriber') && !str_contains($path, '/Saga/')) {
                continue;
            }
            ...
            if ($ref->getAttributes(AsEventSubscriber::class) !== []) {
                $found[] = $class;
            }
        }

        // Also include any attribute-based classes regardless of path
        return $found;
    }
        ...
        try {
            if (class_exists(Discover::class)) {
                ...
                $classes = Discover::in(base_path('src'))
                    ->attributes(AsEventSubscriber::class)
                    ->classes();
